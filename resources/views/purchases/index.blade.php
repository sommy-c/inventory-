...
<div class="customers-page">
    <div class="page-header">
        ...
        <form action="{{ route('admin.purchases.index') }}" method="GET" class="search-form">
            <input type="text" name="supplier" value="{{ $supplierFilter }}" placeholder="Supplier name">
        ...
        btn.addEventListener('click', function (e) {
            ...
            .then(data => {
                ...
                p.items.forEach(function (item) {
                    ...
                    html += '    <td>' + Number(item.cost_price).toFixed(2) + '</td>';
                    html += '    <td>' + Number(item.line_total).toFixed(2) + '</td>';
                ...
                html += '      <tr><td style="padding:4px 8px; text-align:right; color:#9ca3af;">Subtotal</td>' +
                        '<td style="padding:4px 8px; text-align:right;">' + Number(p.subtotal).toFixed(2) + '</td></tr>';
                html += '      <tr><td style="padding:4px 8px; text-align:right; color:#9ca3af;">Discount</td>' +
                        '<td style="padding:4px 8px; text-align:right;">- ' + Number(p.discount).toFixed(2) + '</td></tr>';
                html += '      <tr><td style="padding:4px 8px; text-align:right; color:#9ca3af;">Tax</td>' +
                        '<td style="padding:4px 8px; text-align:right;">+ ' + Number(p.tax).toFixed(2) + '</td></tr>';
                html += '      <tr><td style="padding:4px 8px; text-align:right; font-weight:600;">Total</td>' +
                        '<td style="padding:4px 8px; text-align:right; font-weight:600;">' + Number(p.total).toFixed(2) + '</td></tr>';
                html += '      <tr><td style="padding:4px 8px; text-align:right; color:#9ca3af;">Amount Paid</td>' +
                        '<td style="padding:4px 8px; text-align:right;">' + Number(p.amount_paid).toFixed(2) + '</td></tr>';
                html += '      <tr><td style="padding:4px 8px; text-align:right; color:#9ca3af;">Balance</td>' +
                        '<td style="padding:4px 8px; text-align:right;">' + Number(p.balance).toFixed(2) + '</td></tr>';
                html += '    </tbody>';
                html += '  </table>';
                html += '</div>';
